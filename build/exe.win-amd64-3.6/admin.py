    ...
    if((mode == "edit" or mode == "delete") and (selected == ())):
        ...
        if mode == "create":
            ...
            reverse_button = Button(
                root_que,
                text='Add as Reverse Question',
                bg='light yellow',
                activebackground='light yellow',
                command=functools.partial(question_actions, "reverse", indexLevel)
            )
            reverse_button.place(x=291, y=460)
            ...
            delete_button.place(x=490, y=460)
...
def question_actions(mode, index):
    ...
    if((mode == "edit" or mode == "delete" or mode == "reverse") and (selected_que == ())):
        ...
        if mode == "create" :  
            ...
            q = qt[0]
            t = qt[1]
            if os.path.exists("db/reverse_questions.json"):
                with open("db/reverse_questions.json", "r+") as rever:
                    getArray = json.load(rever)
                    if q in getArray:
                        error = "Selected input is already in revesre question array"
                        error_show(error)
                    else:
                        with open("db/reverse_questions.json", "w") as rever:
                            print(getArray)
                            getArray.append(q)
                            json.dump(getArray, rever)
                            rever.close()
                            success = "Question successfully added to Reverse Question Array"
                            success_show(success) 
            else :
                with open("db/reverse_questions.json", "w+") as rev:
                    array = []
                    array.append(q)
                    json.dump(array, rev)
                    rev.close()
